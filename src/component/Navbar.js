...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const currentItem = menuItems.find(
      (item) => item.link === path || (item.dropdown && item.dropdown.some((dropItem) => dropItem.link === path)),
    )
    if (currentItem) {
  ...
  return (
    <nav className="bg-white sticky top-0 z-50 shadow-md" role="navigation" aria-label="Main Navigation">
      {/* Desktop Navigation */}
      <div className="hidden lg:block max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[100px]">
        <div className="flex justify-between items-center h-full">
          ...
          <div className="flex space-x-8">
            {menuItems.map((item) => (
              <div key={item.name} className="relative group" aria-expanded={openDropdown === item.name}>
                <a
                  href={item.link}
      ...
      <div className="lg:hidden">
        <div className="flex justify-between items-center px-4 py-3 border-b border-gray-200">
          <div className="flex-shrink-0 h-[80px]">
            <a href="/">
              <img src={logo || "/placeholder.svg"} alt="ENGTERN" className="h-[270px] w-[230px] mt-[-110px]" />
            </a>
          </div>
          <button
            onClick={toggleMenu}
          ...
          <div className="fixed inset-0 bg-white z-50">
            <div className="flex justify-between items-center px-4 py-3 border-b border-gray-200">
              <div className="flex items-center h-[80px]">
                <img src={logo || "/placeholder.svg"} alt="ENGTERN" className="h-[270px] w-[230px] mt-[-40px]" />
              </div>
              <button
                onClick={toggleMenu}
                className="text-gray-500 hover:text-gray-700"
                ...
                <div key={item.name} className="border-b border-gray-100 last:border-0">
                  ...
                  >
                    <a
                      ...
                      className={`text-xl ${activeItem === item.name ? "text-[#00B6BC]" : "text-gray-900"}`}
                      onClick={() => !item.dropdown && handleClick(item.name)}
                    >
                      {item.name}
                    </a>
                    {item.dropdown && (
                      <Plus
                        className={`h-5 w-5 transition-transform ${openDropdown === item.name ? "rotate-45" : ""}`}
                      />
                    )}
                  </div>
